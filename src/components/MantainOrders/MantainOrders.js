...
const MantainOrders = () => {
  ...
  const handleDelete = (id) => {
    const url = `https://blooming-forest-81529.herokuapp.com/allServices/${id}`;
    fetch(url, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
        ...
        if (data.deletedCount) {
          ...
          const remaining = mantains.filter(
  ...
  return (
    <div className="row">
      ...
      {mantains.map((mantain) => (
        <div className="col-md-6" key={mantain._id}>
          <div className="col">
            <div className="card">
              ...
              <div className="card-body d-flex justify-content-between">
                ...
                <button
                  className="butnn btn"
                  onClick={() => handleDelete(mantain._id)}
                >
                  remove
                </button>
              </div>
